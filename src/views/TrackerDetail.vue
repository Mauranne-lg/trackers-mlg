  ...
  <div class="max-w-4xl mx-auto p-4">
    ...
    >
      <div class="bg-white rounded-lg p-4 md:p-6 w-full max-w-md">
        <h2 id="delete-modal-title" class="text-lg md:text-xl font-bold text-amber-700 mb-4">
    ...
    <div
      v-if="showTrackerDeleteConfirm"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"
      role="dialog"
      aria-labelledby="tracker-delete-modal-title"
    >
      <div class="bg-white rounded-lg p-4 md:p-6 w-full max-w-md">
        <h2 id="tracker-delete-modal-title" class="text-lg md:text-xl font-bold text-amber-700 mb-4">
          Confirmer la suppression du tracker
        </h2>
        <p class="mb-6 text-gray-700">
          Êtes-vous sûr de vouloir supprimer ce tracker et tous ses événements ? Cette action est irréversible.
        </p>
        <div class="flex justify-end space-x-3">
          <button
            @click="cancelDeleteTracker"
            class="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 transition-colors"
          >
            Annuler
          </button>
    ...
    <template v-if="tracker">
      <div class="flex justify-between items-center mb-4">
        <div v-if="!editingTracker" class="flex-1">
          <h1 class="text-2xl md:text-3xl font-bold text-amber-700">{{ tracker.name }}</h1>
      ...
      <div class="flex justify-between items-center mb-6">
        <div class="flex-1">
          <img src="/src/assets/detail-illustration.svg" alt="Illustration" class="w-32 h-32 mx-auto" />
        </div>
      </div>
      ...
      <div class="bg-white p-4 md:p-6 rounded-lg shadow-md mb-6 md:mb-8">
        <div v-if="tracker.fields[0]" class="space-y-4">
          <p class="text-base md:text-lg text-amber-800">
            Type de suivi : <span class="font-medium">{{ tracker.fields[0].type }}</span>
          </p>
          
          <div v-if="tracker.fields[0].type === 'text'" class="space-y-2">
            <label :for="'text-input-' + tracker.id" class="sr-only">Entrez votre texte</label>
            <input
              :id="'text-input-' + tracker.id"
              v-model="textInput"
              type="text"
          ...
          <button
            @click="createEvent"
            :disabled="tracker.fields[0].type === 'text' && !textInput"
            :class="{
      ...
      <div class="bg-amber-50 p-4 md:p-6 rounded-lg shadow-md">
        <h2 class="text-lg md:text-xl font-semibold text-amber-800 mb-4">Événements</h2>
        <div 
        ...
        >
          <div
            ...
            role="listitem"
          >
            <div v-if="editingEvent?.id === event.id" class="flex items-center space-x-2">
              <input
                v-model="editValue"
                type="text"
            ...
            <div v-else>
              <p class="text-amber-800 break-words pr-16">{{ formatEventValue(event) }}</p>
              <p v-if="tracker.fields[0]?.type === 'text'" class="text-xs text-amber-500 mt-1">
          ...
          >
            <img src="/src/assets/empty-events.svg" alt="Aucun événement" class="w-32 h-32 mx-auto mb-4" />
