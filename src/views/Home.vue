...
<template>
  <div class="max-w-4xl mx-auto p-4">
    <template v-if="!isConfigured">
      <div 
        role="alert"
        class="bg-amber-100 border-l-4 border-amber-500 p-4 mb-8"
      >
    ...
    <template v-else>
      <div class="flex justify-between items-center mb-8">
        ...
        <img src="/src/assets/tracker-illustration.svg" alt="Illustration" class="w-32 h-32" />
      </div>
      ...
      >
        <div 
          class="bg-white rounded-lg p-4 md:p-6 w-full max-w-md relative"
          @click.stop
        >
          <button
            @click="closeModal"
          ...
          <h2 id="modal-title" class="text-lg md:text-xl font-bold text-amber-700 mb-4 pr-8">
            Ajouter un événement pour "{{ selectedTracker?.name }}"
          </h2>
          
          <div class="space-y-4">
            <div>
              <label :for="'modal-text-input'" class="block text-sm font-medium text-amber-700 mb-1">
                Texte
              </label>
              <input
                id="modal-text-input"
                ...
                @keyup.enter="submitTextEvent"
              >
            </div>
            
            <button
              @click="submitTextEvent"
              :disabled="!textInput"
      ...
      >
        <div
          ...
          role="listitem"
          @click="() => router.push(`/tracker/${tracker.id}`)"
        >
          <div class="flex items-center space-x-4">
            <div class="min-w-0 flex-1">
              <h2 class="text-base md:text-lg font-medium text-amber-800 truncate group-hover:text-amber-900">
                {{ tracker.name }}
              </h2>
            ...
            <button
              @click.stop="(e) => createEvent(tracker, e)"
            ...
            >
              <PlusIcon class="w-4 h-4 md:w-5 md:h-5" aria-hidden="true" />
            </button>
          </div>
        </div>
        
        <div 
          v-if="trackers.length === 0 && !loading && !error"
          class="text-center py-12 bg-white rounded-lg shadow-md"
        >
          <img src="/src/assets/empty-state.svg" alt="Aucun tracker" class="w-48 h-48 mx-auto mb-4" />
